...
class MCAMatrix(np.matrix):
    ...
    @staticmethod
    def read_copasi_output(filepath, name=None):
        ...
        try:
            for part in parts:
                ...
                name2mat[lines_part[1]] = MCAMatrix(mat, rowvarids=rowvarids, colvarids=colvarids)
        except:
            pass
        if name:
    ...
    @staticmethod
    def read_jws_output(filepath):
        fh = open(filepath)
        lines = fh.readlines()
        fh.close()
        colvarids = [s.strip() for s in lines[0].split(',')[1:]]
        rowvarids = []
        mat = []
        for line in lines[1:]:
            rowvarids.append(line.split(',')[0])
            mat.append([float(s) for s in line.split(',')[1:]])
        mat = MCAMatrix(mat, rowvarids=rowvarids, colvarids=colvarids)
        return mat
...
def get_reordered_net(net):
    ...
    if N.rank() == N.shape[0]: 
        ...
        net2 = net.__class__(net.id, name=net.name)
...
def get_link_mat(net):
    ...
    try:
        ...
        if len(dynvarids) == len(idynvarids):
            ...
            L0 = np.flipud(get_reduced_link_mat(net))
...
def get_concn_elas_mat(net, paramvals=None, normed=False):
    """
...
def get_param_elas_mat(net, paramvals=None):
    ...
    try:
        ...
        for i in range(len(rxnids)):
            ratelaw = net.reactions[i].kineticLaw
            for j in range(len(paramids)):
                paramid = paramids[j]
                ratelaw_subbed = Ex.sub_for_vars(ratelaw, dict(net.assignmentRules.items()))
                mat[i, j] = net.evaluate_expr(Ex.diff_expr(ratelaw_subbed, paramid))
        setattr(net, attr, mat)
        setattr(net, short2long(attr), mat)
