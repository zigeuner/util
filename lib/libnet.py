...
def same_param(net, paramvals, optimizable=True):
    ...
    try:  # a mapping 
        paramvals = libtype.get_values(paramvals, varids)
...
def reset_variable_ids(net, netid=None, func=None, oldid2newid=None):
    ...
    for rxn in net2.reactions:
        setattr(rxn, 'stoichiometry', 
                libtype.change_items(rxn.stoichiometry, func_key=func))
    
    ## update assignment rules
    net2.assignmentRules = libtype.change_items(net2.assignmentRules, 
                                                func_key=func, func_value=func)
    
    ## update rate rules
    net2.rateRules = libtype.change_items(net2.rateRules, 
                                          func_key=func, func_value=func)
